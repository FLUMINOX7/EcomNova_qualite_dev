...
from backend.models_sql import UserModel, ProductModel, OrderModel
...
def get_statistics(db: Session = Depends(get_session)):
    ...
    # Count products
    products_count = db.query(func.count(ProductModel.id)).filter(
        ProductModel.active == True
    ).scalar() or 0
    
    # Count users (excluding admin accounts)
    users_count = db.query(func.count(UserModel.id)).filter(
