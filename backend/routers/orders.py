...
@router.post("/{order_id}/ship", response_model=DeliveryResponse, status_code=status.HTTP_201_CREATED)
def ship_order(
    order_id: str,
    admin_id: str = Depends(require_admin),
    db: Session = Depends(get_session)
...
def refund_order(
    ...
    # Check order status
    if order.status not in [OrderStatusEnum.PAYEE, OrderStatusEnum.ANNULEE, OrderStatusEnum.EXPEDIEE, OrderStatusEnum.LIVREE]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
