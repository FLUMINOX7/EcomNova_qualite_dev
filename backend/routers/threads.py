...
def create_thread(
    ...
    thread = repo.create_thread(
        user_id=user_id, subject=thread_data.subject, order_id=thread_data.order_id
    )

    # Add initial message
    repo.add_message(
        thread_id=thread.id, body=thread_data.initial_message, author_user_id=user_id
    )
...
def list_all_threads(
    admin_id: str = Depends(require_admin), db: Session = Depends(get_session)
):
...
def post_message(
    ...
    if thread.closed:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Thread is closed"
        )
    ...
    message = repo.add_message(
        thread_id=thread_id, body=message_data.body, author_user_id=user_id
    )
