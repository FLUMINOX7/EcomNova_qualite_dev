...
class UserModel(Base):
    ...
    message_threads = relationship("MessageThreadModel", back_populates="user", cascade="all, delete-orphan")
...
class OrderModel(Base):
    ...
    invoice = relationship("InvoiceModel", uselist=False, back_populates="order", cascade="all, delete-orphan")
    delivery = relationship("DeliveryModel", uselist=False, back_populates="order", cascade="all, delete-orphan")
...
class DeliveryModel(Base):
    ...
    address = Column(Text, nullable=False)
    status = Column(Enum(DeliveryStatusEnum), nullable=False, default=DeliveryStatusEnum.PREPAREE)
    created_at = Column(Float, nullable=False)
    shipped_at = Column(Float, nullable=True)
    delivered_at = Column(Float, nullable=True)
...
class MessageModel(Base):
    ...
    id = Column(String, primary_key=True)
    thread_id = Column(String, ForeignKey("message_threads.id"), nullable=False, index=True)
    author_user_id = Column(String, ForeignKey("users.id"), nullable=True)  # None = agent support
