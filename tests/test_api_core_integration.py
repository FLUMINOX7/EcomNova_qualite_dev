...
class TestCoreAuth:
    ...
    def test_register_duplicate_email(self, client, registered_user):
        ...
        assert (
            ...
            or "utilisé" in response.json()["detail"].lower()
        )
    ...
    def test_login_nonexistent_user(self, client):
        ...
        login_data = {"email": "nobody@test.com", "password": "anypassword"}
...
class TestCoreCart:
    ...
    def test_add_item_to_cart(self, client, registered_user, sample_product):
        ...
        headers = {"Authorization": f"Bearer {registered_user['token']}"}
        cart_item = {"product_id": sample_product.id, "quantity": 3}
...
class TestCoreOrders:
    ...
    def test_checkout_empty_cart(self, client, registered_user):
        ...
        assert response.status_code == 400  # Bad Request
        assert (
            "panier" in response.json()["detail"].lower()
            or "vide" in response.json()["detail"].lower()
        )
